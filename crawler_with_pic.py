...
def urls_from_excel_main_content(excel_file, sheet_name, url_column, output_folder):
    ...
    for idx, url in enumerate(urls, start=1):
        try:
            ...
            if not main_content:
                main_content = soup  # fallback to entire page if main not found

            doc = Document()
            doc.add_heading(page_title, level=0)
            doc.add_paragraph(f"Source URL: {url}")

            # Extract unique text from main content
            ...
            # Extract and add images from main content (limit to 5)
            ...
            doc.save(file_path)
            print(f"Saved main content: {file_path}")
...
urls_from_excel_main_content(excel_file, sheet_name, url_column, output_folder)
